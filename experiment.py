...
from state_norm_params import (
    state_norm_params,
)  # we use normalization parameter for states from the behavioral policy
...
def pref_grid(n_obj, max_prefs=None, min_prefs=None, granularity=5):
    ...
    for _ in range(n_obj):
        prefs = [
            ...
            for x in prefs
            for y in grid
            if sum(x + [y]) < 1 or isCloseToOne(sum(x + [y]))
        ]
...
def experiment(variant):
    ...
    for traj in trajectories:
        if concat_state_pref != 0:
            traj["observations"] = np.concatenate(
                (traj["observations"], np.tile(traj["preference"], concat_state_pref)),
                axis=1,
            )
    ...
    if not isCloseToOne(percent_dt):
        ...
        indices_wanted = np.unique(
            np.argpartition(returns_mo, -num_traj_wanted, axis=0)[-num_traj_wanted:]
        )
    ...
    if (
        concat_act_pref == 0
        and concat_rtg_pref == 0
        and concat_state_pref == 0
        and model_type == "bc"
    ):
        granularity = 1
    ...
    if (not pref_loss) and (not return_loss):
        loss_fn = lambda s_hat, a_hat, r_hat, pref_hat, s, a, r, pref: torch.mean(
            (a_hat - a) ** 2
        )
    # alternatively, can train on predicting preference
        ...
        loss_fn = lambda s_hat, a_hat, r_hat, pref_hat, s, a, r, pref: torch.mean(
            (a_hat - a) ** 2
        ) + torch.mean((r_hat - r) ** 2)
    elif pref_loss and (not return_loss):
        loss_fn = lambda s_hat, a_hat, r_hat, pref_hat, s, a, r, pref: torch.mean(
            (a_hat - a) ** 2
        ) + torch.mean((pref_hat - pref) ** 2)
    else:
        loss_fn = (
            lambda s_hat, a_hat, r_hat, pref_hat, s, a, r, pref: torch.mean(
                (a_hat - a) ** 2
            )
            + torch.mean((r_hat - r) ** 2)
            + torch.mean((pref_hat - pref) ** 2)
    ...
    trainer = Trainer(
        ...
        eval_fns=eval_episodes(
            pref_set=prefs
        ),  # this return a list (of lists) of eval_fns
        max_iter=max_iters,
    ...
    for iter in range(max_iters):
        ...
        torch.save(
            {"model": model.state_dict(), "optimizer": optimizer.state_dict()}, filename
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_obj", type=int, default=-1
    )  # decay to only 1-obj scenario. -1 default means nothing is decayed
    parser.add_argument(
        "--percent_dt", type=float, default=1
    )  # make DT to only use top% of data, default would be 99%
    ...
    if args.log_to_wandb:
        wandb.init(project=args.wandb_group, entity="baitingz", name=args.run_name)
